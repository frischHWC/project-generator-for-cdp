...
def render_code_files(env: Environment, language: str, feature, logger_needed: bool, package_name: str,  user: str,
                      hdfs_nameservice: str, hdfs_work_dir: str, components):
    """
    ...
    write_template_to_file(
        ...
        target_path + "/App." + language_extension)

    write_template_to_file(
            env.get_template("code/" + language + "/Treatment." + language_extension)
               .render(language=language, feature=feature, logger=logger_needed, package_name=package_name,
                       components=components),
            target_path + "/Treatment." + language_extension)
    ...
    write_template_to_file(
            env.get_template("code/" + language + "/AppConfig." + language_extension)
               .render(feature=feature, package_name=package_name, user=user, hdfsNameservice=hdfs_nameservice,
                       hdfsWorkDir=hdfs_work_dir, components=components),
            target_path + "/AppConfig." + language_extension)
...
def render_compiler_files(env: Environment, compiler: str, version: str, feature, project_name: str, language: str,
                          package_name: str, components, logger_enabled: bool, libs, fat_jar: bool):
    """
        ...
        write_template_to_file(
            env.get_template("compiler/maven/pom.xml")
               .render(version=version, feature=feature, project_name=project_name, language=language,
                       package_name=package_name, components=components, logger=logger_enabled, libs=libs, fat_jar=fat_jar),
            target_path + "/pom.xml")
    elif compiler == "sbt":
        write_template_to_file(
            env.get_template("compiler/sbt/build.sbt")
            .render(version=version, feature=feature, project_name=project_name, components=components, logger=logger,
                    libs=libs, fat_jar=fat_jar),
            target_path + "/build.sbt"
        )
        write_template_to_file(
            env.get_template("compiler/sbt/assembly.sbt")
               .render(),
            target_path + "/assembly.sbt"
        )
        write_template_to_file(
            env.get_template("compiler/sbt/build.properties")
               .render(),
            target_path + "/build.properties"
        )
        write_template_to_file(
            env.get_template("compiler/sbt/plugins.sbt")
               .render(),
            target_path + "/plugins.sbt"
        )
...
def render_script_files(env: Environment, language: str, feature, kerberos: bool, project_name: str,
                        logger_needed: bool, package_name: str, compiler: str, principal: str, keytab: str,
                        host: str, user: str, fat_jar: bool):
    """
    ...
    write_template_to_file(
            ...
            target_path + "/launchToCDP.sh")

    write_template_to_file(
        env.get_template("scripts/launch.sh")
            .render(language=language, project_name=project_name, user=user),
        target_path + "/launch.sh")

    write_template_to_file(
        env.get_template("scripts/launchFromIDE.sh")
            .render(host=host, language=language),
        target_path + "/launchFromIDE.sh")

    logger.debug("Generated script files for language : %s " +
                 ", feature : %s, kerberos : %s, logger : %s", language, feature, kerberos, logger_needed)


def render_configuration_files(env: Environment, language: str, feature, project_name: str, user: str,
                               hdfs_nameservice: str, logger_needed: bool, hdfs_work_dir: str, components, libs):
    """
    ...
    if language == "scala" or "typesafe" in libs:
        write_template_to_file(
            ...
            .render(feature=feature, project_name=project_name, user=user,
                    hdfsNameservice=hdfs_nameservice, hdfsWorkDir=hdfs_work_dir, components=components),
                    target_path + "/application.conf")
    elif language == "java":
        write_template_to_file(
            env.get_template("configuration/application.properties")
                .render(feature=feature, project_name=project_name, user=user,
                        hdfsNameservice=hdfs_nameservice, hdfsWorkDir=hdfs_work_dir, components=components),
            target_path + "/application.properties")

    # TODO: Add python configuration from file
    logger.debug("Generated configuration files for language : %s", language)


def render_doc_files(env: Environment, language: str, compiler: str, version: str, kerberos: bool,
                     project_name: str, doc_type: str, components):
    """
    ...
    write_template_to_file(
        env.get_template("docs/README." + doc_type)
           .render(language=language, compiler=compiler, kerberos=kerberos,
                   version=version, project_name=project_name, components=components),
        target_path + "/README." + doc_type)
